...
const ResponseCard: React.FC<ResponseCardProps> = ({
  ...
  const calculateTps = () => {
    ...
    return (out / durationInSeconds).toFixed(2);
  };
  ...
  return (
    <GlassCard className="p-4 flex flex-col h-full">
      ...
      {metrics && (
        <div className="mt-4 pt-2 border-t border-white/10 text-xs text-gray-400 grid grid-cols-4 gap-2">
          <div>
          ...
          <div>
            ...
            {typeof metrics.outputTokens === 'number' ? metrics.outputTokens : '—'}
          </div>
        </div>
      )}
    </GlassCard>
